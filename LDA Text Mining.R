...
library(LDAvis); library(slam)

# --------- fetch the Reddit thread JSON (replace username in user_agent) ----------
...
# --------- collect comments (skip "more" nodes) ----------
...
# --------- simple cleaning + tokenization (tidytext) ----------
...
# --------- build Document-Term Matrix (tm::DocumentTermMatrix) ----------
dtm <- tokens %>%
...
# --------- document-topic probabilities (gamma) & top docs per topic ----------
...
# ---------------- Save summary outputs to CSV (macOS Desktop) ----------------
...
# OPTIONAL: write full document-term matrix (may be large) — uncomment if desired
